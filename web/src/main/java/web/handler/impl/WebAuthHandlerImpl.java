...
//        }
    ...
    public void handle( RoutingContext ctx ){
        ctx.addHeadersEndHandler( v -> ctx.response().putHeader( "Content-Type", "application/json" ) );

        final CustomWebUser user = (CustomWebUser) ctx.user();

        if( user == null ) {
            ctx.put( "e", ErrorCode.USER_NOT_LOGIN ).fail( 403 );
            return;
        }
        ...
        if( permissionOrRole == null ) {
            ctx.put( "e", ErrorCode.SYS_HANDLER_NOT_FOUND ).fail( 403 );
            return;
        }
